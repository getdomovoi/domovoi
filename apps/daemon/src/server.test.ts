...
function authenticatedSocket(
  ...
  // immutable hello immediately before the first ordinary RPC and hide only
  // that helper response. Authentication/identity tests use a raw WebSocket,
  // so they still exercise the unauthenticated boundary directly.
...
function identifyClient(
  ...
  return new Promise((resolve) => {
    ...
    socket.send(JSON.stringify({
      ...
      id,
      method: "system.hello",
      params: { client, clientId, clientVersion: "0.0.1", protocolVersion },
    }))
  })
}
...
function identifyMachine(socket: WebSocket, machineId: string): Promise<void> {
  return new Promise((resolve) => {
    ...
    socket.send(JSON.stringify({
      ...
      id,
      method: "system.hello",
      params: { client: "machine", machineId, clientVersion: "0.0.1", protocolVersion },
    }))
  })
}
...
describe("DomovoiDaemon", () => {
  ...
  it("closes a client whose RPC response reaches the outbound high-water boundary", async () => {
    const daemon = new DomovoiDaemon({
      ...
      rpcOutboundBackpressure: {
        // Admit the protocol hello, then make the requested workspace response
        // hit the high-water boundary this assertion is about.
        bufferedBytes: vi.fn()
          .mockReturnValueOnce(0)
          .mockReturnValue(rpcWebSocketHighWaterBytes),
      },
    })
    running.push(daemon)
  ...
  it("bypasses ordinary pressure policy for terminal notifications", async () => {
    ...
    const daemon = new DomovoiDaemon({
      ...
      rpcOutboundBackpressure: {
        // The protocol hello must complete before this test applies pressure
        // to the terminal notification and its RPC response.
        bufferedBytes: vi.fn()
          .mockReturnValueOnce(0)
          .mockReturnValue(rpcWebSocketHighWaterBytes),
      },
  ...
  it("broadcasts the snapshot as persisted, including deltas streamed during the persist", async () => {
    const snapshot = structuredClone(demoWorkspace)
    snapshot.approvals = []
    ...
    socket.on("message", (data) => {
      ...
      else if (message.method === "workspace.delta" || message.method === "workspace.changed") {
    ...
    const assistantBodies = (payload: Record<string, unknown>) =>
      (payload.thread as { sessionId: string; kind: string; body?: string }[])
    ...
    const turnId = (sent.result as { sessions: { id: string; activeTurnId?: string }[] }).sessions
      .find((session) => session.id === streaming.id)!.activeTurnId!
    ...
    const daemonView = await rpc("workspace.get", {})
    expect(assistantBodies(changed.params)).toEqual(
      assistantBodies(daemonView.result as Record<string, unknown>),
    )
    socket.close()
  })
  ...
  it("accounts late and duplicate usage against dispatch after a same-provider model change", async () => {
    ...
    expect(await rpc("session.usage", { sessionId: session.id })).toMatchObject({
      totalTokens: 44, coverage: { complete: 2, pending: 0 },
    })
  ...
  it("serializes every session route with the same session resource", async () => {
    const snapshot = structuredClone(demoWorkspace)
    const session = snapshot.sessions[0]!
    session.workspacePath = "/worktrees/session-evidence"
    ...
    const retiredTransfer = responseFor(3)
    socket.send(JSON.stringify({
      jsonrpc: "2.0",
      id: 1,
      method: "session.evidence",
    ...
    // Retired methods still pass through resource selection before RPC
    // validation. Keep this former session route on the session queue so it
    // cannot race an in-flight mutation if an older client calls it.
    socket.send(JSON.stringify({
      jsonrpc: "2.0",
      id: 3,
    ...
    await expect(retiredTransfer).resolves.toMatchObject({
      id: 3,
      error: { code: -32601, message: "Unknown method: transfer.fromRef" },
    })
    expect(retiredBeforeEvidence).toBe(false)
  ...
  it("reports the persisted machine identity to clients", async () => {
    ...
    socket.send(JSON.stringify({
      ...
      id: 1,
      method: "system.hello",
      params: { client: "web", clientVersion: "0.0.1", protocolVersion },
    }))
  ...
  it("accepts a paired device credential and rejects it once revoked", async () => {
    ...
    const hello = (socket: WebSocket) => {
      ...
      socket.send(JSON.stringify({
        ...
        id: 1,
        method: "system.hello",
        params: { client: "tablet", clientVersion: "0.0.1", protocolVersion },
      }))
      return response
    }
  ...
  it.each([
    ...
    await expect(send(1, "system.hello", { client: "tablet", clientVersion: "0.0.1", protocolVersion }))
  ...
  async function pairingClient(daemon: DomovoiDaemon, token?: string) {
    ...
    await call(1, "system.hello", { client: "desktop", clientVersion: "0.0.1", protocolVersion })
    return { socket, call }
  }
  ...
  it("serves an encrypted listener when given TLS material", async () => {
    ...
    socket.send(JSON.stringify({
      ...
      id: 1,
      method: "system.hello",
      params: { client: "desktop", clientVersion: "0.0.1", protocolVersion },
    }))
  ...
  it("refuses a versionless hello as protocol 0.1.0", async () => {
    const daemon = new DomovoiDaemon({
      port: 0,
      store: new SqliteWorkspaceStore(":memory:", structuredClone(demoWorkspace)),
      authToken: testAuthToken("correct-horse-battery-staple"),
  ...
  it("quarantines a provider thread that refuses to stop during a switch", async () => {
    const scratch = await mkdtemp(join(tmpdir(), "domovoi-per-project-"))
    scratchDirectories.push(scratch)
    const statePath = join(scratch, "state.sqlite")
    const agentListeners = new Set<(event: AgentEvent) => void>()
    ...
    // A thread that would not stop may still be writing to the worktree, so the
    // session must come back asking for a restart rather than looking idle.
    // The project just switched to holds no sessions, so returning needs no
    // confirmation.
  ...
  it("posts the Codex sandbox notice when a session starts on, is handed off to, or is forked to Codex", async () => {
    ...
    const workspaceService = {
      ...
      createSessionWorkspaceFromCheckpoint: vi.fn(async (_source: string, commit: string, sessionId: string) => ({
        path: `/worktrees/${sessionId}`,
        branch: `domovoi/${sessionId}`,
        baseCommit: commit,
      })),
    ...
    await rpc("project.open", { path: "/code/domovoi", client: "desktop" })
    const onClaude = await rpc("session.create", { title: "Claude session", runtime: claudeRuntime, client: "desktop" })
    const claudeSessionId = onClaude.activeSessionId!
    expect(noticesFor(onClaude, claudeSessionId)).toEqual([])

    const onCodex = await rpc("session.create", { title: "Codex session", runtime: codexRuntime, client: "desktop" })
    const codexSessionId = onCodex.activeSessionId!
    expect(noticesFor(onCodex, codexSessionId)).toEqual([expect.objectContaining(notice)])

    const checkpointId = onClaude.thread.find(
      (item) => item.sessionId === claudeSessionId && item.kind === "checkpoint",
    )!.id
    const forkedToClaude = await rpc("session.fork", {
      sessionId: claudeSessionId,
      checkpointId,
      requestId: "fork-to-claude",
      runtime: claudeRuntime,
      client: "desktop",
    })
    const claudeFork = forkedToClaude.sessions.find((session) => session.forkedFrom?.requestId === "fork-to-claude")!
    expect(noticesFor(forkedToClaude, claudeFork.id)).toEqual([])
    const forkedToCodex = await rpc("session.fork", {
      sessionId: claudeSessionId,
      checkpointId,
      requestId: "fork-to-codex",
      runtime: codexRuntime,
      client: "desktop",
    })
    const codexFork = forkedToCodex.sessions.find((session) => session.forkedFrom?.requestId === "fork-to-codex")!
    expect(noticesFor(forkedToCodex, codexFork.id)).toEqual([expect.objectContaining(notice)])
    ...
    expect(noticesFor(handedOff, claudeSessionId)).toEqual([expect.objectContaining(notice)])
    const handedBack = await rpc("session.setRuntime", {
      sessionId: codexSessionId,
      runtime: claudeRuntime,
      client: "desktop",
    })
    expect(noticesFor(handedBack, codexSessionId)).toHaveLength(1)
    socket.close()
  })
  ...
  it("forks a checkpoint idempotently without mutating the source selection", async () => {
    ...
    expect(createdResult.thread.filter((item) => item.sessionId === fork.id)).toEqual([
      ...
      expect.objectContaining({ kind: "system", body: "Codex cannot read secret files in this worktree." }),
    ])
  ...
  // Ruled for #541 and applied here: a card whose file is hidden as [REDACTED]
  // is a hard gate. No standing rule answers it, it offers no Always, and a card
  // whose target moves to such a file becomes one when it is read again.
  it("makes a card whose file is hidden a hard gate, when raised and when its target moves", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "domovoi-file-hidden-"))
    scratchDirectories.push(workspacePath)
    await mkdir(join(workspacePath, "one"), { recursive: true })
    await mkdir(join(workspacePath, ".ssh"), { recursive: true })
    const snapshot = structuredClone(demoWorkspace)
    const session = snapshot.sessions[0]!
    session.runtime = {
      provider: "claude-code",
    ...
    session.state = "idle"
    session.workspacePath = workspacePath
    session.providerThreadId = "thread-file-hidden"
    delete session.activeTurnId
    snapshot.approvals = []
    snapshot.approvalRules = []
    ...
    const agent = {
      ...
      resumeThread: vi.fn(async () => {}),
      stopThread: vi.fn(async () => {}),
      startTurn: vi.fn(async () => "turn-file-hidden"),
      steerTurn: vi.fn(async () => {}),
      interruptTurn: vi.fn(async () => {}),
      resolveApproval: vi.fn(),
      onEvent: vi.fn((next: (event: AgentEvent) => void) => {
    ...
    type Card = { id: string; providerRequestId?: number; risk: string; affects: string; revision: number }
    const cards = async () => ((await rpc("workspace.get", {})).result as { approvals: Card[] }).approvals
    const card = async (requestId: number) => (await cards()).find((candidate) => candidate.providerRequestId === requestId)!
    const hidden = "The file [REDACTED] in the session worktree."
    const hardGateRule = "Hard-gate approvals cannot create standing rules"

    await rpc("session.send", { sessionId: session.id, prompt: "Edit the files", client: "desktop" })
    // The standing rule names this file, and still does not answer it.
    listener!({
      type: "approval-requested",
      requestId: 71,
      threadId: session.providerThreadId,
      turnId: "turn-file-hidden",
      reason: "Edit a file",
      command: "Edit",
    ...
    expect(agent.resolveApproval).not.toHaveBeenCalled()
    const named = await card(71)
    expect(named).toMatchObject({ risk: "hard-gate", affects: hidden, revision: 0 })
    await expect(rpc("approval.resolve", { approvalId: named.id, decision: "always-project", revision: 0, client: "desktop" }))
      .resolves.toMatchObject({ error: { message: hardGateRule } })
    expect(await card(72)).toMatchObject({ risk: "normal", affects: "The file one/config in the session worktree.", revision: 0 })

    // While the second card waits, "one" becomes a link to the .ssh directory.
    await rename(join(workspacePath, "one"), join(workspacePath, "one-before"))
    await symlink(join(workspacePath, ".ssh"), join(workspacePath, "one"), "junction")
    const moved = await card(72)
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "allow-once", revision: 0, client: "desktop" }))
      .resolves.toMatchObject({ error: { message: "The file target changed; review the updated approval before allowing it" } })
    expect(await card(72)).toMatchObject({ risk: "hard-gate", affects: hidden, revision: 1 })
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "always-project", revision: 1, client: "desktop" }))
      .resolves.toMatchObject({ error: { message: hardGateRule } })
    expect(agent.resolveApproval).not.toHaveBeenCalled()
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "allow-once", revision: 1, client: "desktop" }))
      .resolves.not.toHaveProperty("error")
    expect(agent.resolveApproval).toHaveBeenCalledWith(72, "allow-once")
    const rules = ((await rpc("workspace.get", {})).result as { approvalRules: Array<{ id: string; useCount: number }> }).approvalRules
    expect(rules).toEqual([expect.objectContaining({ id: "rule-env-edit", useCount: 0 })])
    socket.close()
  })
  ...
  // An unresolved card is read again on Allow too: a hard-linked file can be
  // replaced by a link to somewhere else while its card waits.
  it("reads an unresolved file card again on Allow and rewrites it when its target moved", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "domovoi-file-unresolved-"))
    const outside = await mkdtemp(join(tmpdir(), "domovoi-file-unresolved-outside-"))
    scratchDirectories.push(workspacePath, outside)
    await mkdir(join(workspacePath, "src"), { recursive: true })
    await writeFile(join(outside, "notes.txt"), "outside")
    for (const name of ["moved.json", "kept.json"]) {
      await writeFile(join(workspacePath, "src", name), "{}")
      await link(join(workspacePath, "src", name), join(workspacePath, `${name}.twin`))
    }
    const snapshot = structuredClone(demoWorkspace)
    const session = snapshot.sessions[0]!
    session.runtime = {
    ...
    session.state = "idle"
    session.workspacePath = workspacePath
    session.providerThreadId = "thread-file-unresolved"
    delete session.activeTurnId
    snapshot.approvals = []
    snapshot.approvalRules = []
    let listener: ((event: AgentEvent) => void) | undefined
    const agent = {
      ...
      resumeThread: vi.fn(async () => {}),
      stopThread: vi.fn(async () => {}),
      startTurn: vi.fn(async () => "turn-file-unresolved"),
      steerTurn: vi.fn(async () => {}),
      interruptTurn: vi.fn(async () => {}),
      resolveApproval: vi.fn(),
      onEvent: vi.fn((next: (event: AgentEvent) => void) => {
    ...
    await rpc("session.send", { sessionId: session.id, prompt: "Edit the config", client: "desktop" })
    for (const [requestId, name] of [[61, "moved.json"], [62, "kept.json"]] as const) {
      listener!({
        type: "approval-requested",
        requestId,
        threadId: session.providerThreadId,
        turnId: "turn-file-unresolved",
        reason: "Edit a file",
        command: "Edit",
        path: join(workspacePath, "src", name),
      })
    ...
    await vi.waitFor(async () => expect(await cards()).toHaveLength(2), { timeout: 3_000 })
    const moved = await card(61)
    expect(moved).toMatchObject({
      risk: "normal",
      affects: "The file src/moved.json in the session worktree.",
      revision: 0,
      execution: { state: "unresolved", reason: "unsupported-syntax" },
    })

    // While the card waits, the hard-linked file becomes a link to a file outside.
    await unlink(join(workspacePath, "src", "moved.json"))
    await symlink(join(outside, "notes.txt"), join(workspacePath, "src", "moved.json"))
    const outsideAffects = `The file ${join(await realpath(outside), "notes.txt")}, outside the session worktree, through a link at src/moved.json.`
    const broadcastsBefore = broadcasts.length
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "allow-once", revision: 0, client: "desktop" }))
      .resolves.toMatchObject({ error: { message: "The file target changed; review the updated approval before allowing it" } })
    expect(agent.resolveApproval).not.toHaveBeenCalledWith(61, expect.anything())
    const rewritten = {
      affects: outsideAffects,
      revision: 1,
      execution: { state: "unresolved", reason: "cwd-outside-project" },
    }
    expect(await card(61)).toMatchObject(rewritten)
    expect(broadcasts.slice(broadcastsBefore).some((approvals) => approvals.some(
      (candidate) => candidate.id === moved.id && candidate.revision === 1 && candidate.affects === outsideAffects,
    ))).toBe(true)
    expect(store.save).toHaveBeenCalledWith(expect.objectContaining({
      approvals: expect.arrayContaining([expect.objectContaining({ id: moved.id, ...rewritten })]),
    }))

    // The rewritten card is still unresolved, so it offers no Always, and an
    // Allow at the revision it now shows is released.
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "always-project", revision: 1, client: "desktop" }))
      .resolves.toMatchObject({ error: { message: "Unresolved commands cannot create standing rules" } })
    await expect(rpc("approval.resolve", { approvalId: moved.id, decision: "allow-once", revision: 1, client: "desktop" }))
      .resolves.not.toHaveProperty("error")
  ...
  // A directory or a FIFO put where the file was keeps the path, so only the
  // kind of what is there now tells the Allow that the target changed.
  it("refuses an Allow for a file target replaced by a directory or a FIFO while its card waits", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "domovoi-file-nonregular-"))
    scratchDirectories.push(workspacePath)
    await mkdir(join(workspacePath, "src"), { recursive: true })
    // Windows has no named pipe in the file tree, so there it checks the directory alone.
    const replaced = process.platform === "win32"
      ? [[81, "folder.json"]] as const
      : [[81, "folder.json"], [82, "pipe.json"]] as const
    for (const [, name] of replaced) await writeFile(join(workspacePath, "src", name), "{}")
    const snapshot = structuredClone(demoWorkspace)
    const session = snapshot.sessions[0]!
    session.runtime = {
      provider: "claude-code",
    ...
    session.state = "idle"
    session.workspacePath = workspacePath
    session.providerThreadId = "thread-file-nonregular"
    delete session.activeTurnId
    snapshot.approvals = []
    snapshot.approvalRules = []
    let listener: ((event: AgentEvent) => void) | undefined
    const agent = {
      ...
      resumeThread: vi.fn(async () => {}),
      stopThread: vi.fn(async () => {}),
      startTurn: vi.fn(async () => "turn-file-nonregular"),
      steerTurn: vi.fn(async () => {}),
      interruptTurn: vi.fn(async () => {}),
      resolveApproval: vi.fn(),
      onEvent: vi.fn((next: (event: AgentEvent) => void) => {
    ...
    await rpc("session.send", { sessionId: session.id, prompt: "Edit the config", client: "desktop" })
    for (const [requestId, name] of replaced) {
      listener!({
        type: "approval-requested",
        requestId,
        threadId: session.providerThreadId,
        turnId: "turn-file-nonregular",
        reason: "Edit a file",
        command: "Edit",
        path: join(workspacePath, "src", name),
      })
    }
    await vi.waitFor(async () => expect(await cards()).toHaveLength(replaced.length), { timeout: 3_000 })
    for (const [requestId] of replaced) {
      expect(await card(requestId)).toMatchObject({ revision: 0, execution: { state: "resolved" } })
    }

    // While the cards wait, one file becomes a directory and the other a FIFO.
    const { execFileSync } = await import("node:child_process")
    await unlink(join(workspacePath, "src", "folder.json"))
    await mkdir(join(workspacePath, "src", "folder.json"))
    if (process.platform !== "win32") {
      await unlink(join(workspacePath, "src", "pipe.json"))
      execFileSync("mkfifo", [join(workspacePath, "src", "pipe.json")])
    }
    for (const [requestId, name] of replaced) {
      ...
      expect(agent.resolveApproval).not.toHaveBeenCalledWith(requestId, expect.anything())
      const rewritten = {
        affects: `The file src/${name} in the session worktree.`,
        revision: 1,
        execution: { state: "unresolved", reason: "unsupported-syntax" },
      }
      expect(await card(requestId)).toMatchObject(rewritten)
      expect(broadcasts.slice(broadcastsBefore).some((approvals) => approvals.some(
        (candidate) => candidate.id === shown.id && candidate.revision === 1,
      ))).toBe(true)
      expect(store.save).toHaveBeenCalledWith(expect.objectContaining({
        approvals: expect.arrayContaining([expect.objectContaining({ id: shown.id, ...rewritten })]),
      }))
      // The rewritten card is unresolved, so it offers no Always.
      await expect(rpc("approval.resolve", { approvalId: shown.id, decision: "always-project", revision: 1, client: "desktop" }))
        .resolves.toMatchObject({ error: { message: "Unresolved commands cannot create standing rules" } })
    }
    expect(agent.resolveApproval).not.toHaveBeenCalled()
    const rules = ((await rpc("workspace.get", {})).result as { approvalRules: unknown[] }).approvalRules
  ...
  it("hides the path a card hides in its operation and command text, and nothing else", async () => {
    ...
    // A chain of two links into a credential store (final check after
    // 59484617): hop1 leads to hop2 as an absolute path, hop2 to .ssh as a
    // relative one.
    ...
    // A credential file reached through the two-link chain, named at the
    // first link's target, where the second link still lies ahead.
    listener!({
      ...request,
      requestId: 508,
    ...
    expect(await card(508)).toMatchObject({
      risk: "hard-gate",
      operation: "Edit [REDACTED], which is [REDACTED] or [REDACTED]",
      command: "Edit",
    })
  ...
  it("forgets a file card's requested path when the card leaves without an answer", async () => {
    ...
    session.state = "idle"
    session.workspacePath = workspacePath
    session.providerThreadId = "thread-file-leaves"
    delete session.activeTurnId
    snapshot.approvals = []
    snapshot.approvalRules = []
    let listener: ((event: AgentEvent) => void) | undefined
    const agent = {
      ...
      resumeThread: vi.fn(async () => {}),
      stopThread: vi.fn(async () => {}),
      startTurn: vi.fn(async () => "turn-file-leaves"),
      steerTurn: vi.fn(async () => {}),
      interruptTurn: vi.fn(async () => {}),
      resolveApproval: vi.fn(),
      onEvent: vi.fn((next: (event: AgentEvent) => void) => {
    ...
    type Card = { id: string; execution: { state: string } }
    const cards = async () => ((await rpc("workspace.get", {})).result as { approvals: Card[] }).approvals

    await rpc("session.send", { sessionId: session.id, prompt: "Edit the entry point", client: "desktop" })
    listener!({
      type: "approval-requested",
      requestId: 61,
      threadId: session.providerThreadId,
      turnId: "turn-file-leaves",
      reason: "Edit a file",
      command: "Edit",
      path: join(workspacePath, "src", "index.ts"),
    })
    ...
    expect(card!.execution.state).toBe("resolved")
    expect(daemon.fileApprovalTargetIds).toEqual([card!.id])
  ...
  it("serves attributed redacted audit records for authenticated RPC outcomes", async () => {
    ...
    await expect(rpc("system.hello", {
      ...
      clientVersion: "audit-test", protocolVersion,
      authToken: daemon.authToken,
    })).resolves.toHaveProperty("result")
...
describe("DomovoiDaemon session transfer requests", () => {
  ...
  const transferBaseCommit = "a".repeat(40)
  const transferCheckpointCommit = "b".repeat(40)

  function stubWorkspaceService() {
    let checkpointed = false
    return {
      inspect: async () => ({ root: "/repo", name: "repo", branch: "main", head: transferBaseCommit }),
      createSessionWorkspace: async (_repository: string, sessionId: string) => ({
        path: `/worktrees/${sessionId}`,
        branch: `domovoi/${sessionId}`,
        baseCommit: transferBaseCommit,
      }),
      removeSessionWorkspace: async () => {},
      checkpoint: async () => {
        checkpointed = true
        return { commit: transferCheckpointCommit, changedFiles: [] }
      },
      restore: async () => ({
        restoredCommit: transferCheckpointCommit,
        recoveryCommit: "c".repeat(40),
      }),
      transferFingerprint: async () => ({
        headCommit: checkpointed ? transferCheckpointCommit : transferBaseCommit,
      ...
      restoreSessionFromBundle: async (_bundlePath: string, sessionId: string) => ({
        path: `/worktrees/${sessionId}`,
        branch: `domovoi/${sessionId}`,
        baseCommit: transferCheckpointCommit,
      }),
    }
  }

  function emptyTransferTargetSnapshot(targetMachineId: string) {
    const snapshot = structuredClone(demoWorkspace)
  ...
  async function approvedSessionTransfer(
    call: ReturnType<typeof rpcCaller>,
    params: {
      sessionId: string
    ...
    const preview = await call("session.transferPreview", params)
    expect(preview).toMatchObject({
      result: {
        allowed: true,
        contractVersion: 2,
  ...
  function transferDaemon(options: {
    ...
    const session = snapshot.sessions.find((candidate) => candidate.state === "idle")!
    session.workspacePath = `/worktrees/${session.id}`
    session.baseCommit = transferBaseCommit
    session.ownershipGeneration = 0
    ...
    running.push(daemon)
    return { daemon, store, session }
  }

  it("moves a session to another daemon with no dialer supplied", async () => {
    const targetMachineId = `machine-${"e".repeat(32)}`
    const restored: string[] = []
    const targetStore = new SqliteWorkspaceStore(
      ":memory:",
      emptyTransferTargetSnapshot(targetMachineId),
    )
    const targetCredential = targetStore.devices.pair({
      label: "source daemon",
      binding: { kind: "machine", machineId: sourceMachineId },
    }).token
    ...
    moved.baseCommit = transferBaseCommit
    moved.ownershipGeneration = 0
    // The machine id must be one the transfer protocol accepts, and the
    // project belongs to that machine.
    snapshot.machine.id = sourceMachineId
    if (snapshot.project) snapshot.project.machineId = snapshot.machine.id
    const store = new SqliteWorkspaceStore(":memory:", snapshot)
    const credentials = new Map<string, string>([[targetMachineId, targetCredential]])
    const source = new DomovoiDaemon({
      ...
      workspaceService: {
        ...
        bundleSession: async (_worktree: string, bundlePath: string) => ({
          path: bundlePath,
          commit: transferCheckpointCommit,
          incremental: false,
        }),
      },
    ...
    const source = new DomovoiDaemon({
      ...
      workspaceService: {
        ...
        pushSessionRef: async (_worktree: string, remote: string, sessionId: string) => {
          ...
          return {
            ref: `refs/domovoi/sessions/${sessionId}`,
            commit: transferCheckpointCommit,
            remote,
          }
        },
    ...
    const source = new DomovoiDaemon({
      ...
      connectToMachine: async () => ({
        call: async (method: string) => method === "transfer.preflight"
          ? {
              allowed: true,
              targetProjectId: "project-transfer-target",
              lineageCommit: transferBaseCommit,
            }
          : {},
        close: () => {},
      }),
  ...
  function fakeCodexAgent(listeners: Set<(event: AgentEvent) => void>) {
    return {
      connect: vi.fn(async () => {}), listModels: vi.fn(async () => codexModels()),
      startThread: vi.fn(async () => "unused"), resumeThread: vi.fn(async () => {}),
      stopThread: vi.fn(async () => {}), startTurn: vi.fn(async () => "turn-streaming"),
      steerTurn: vi.fn(async () => {}), interruptTurn: vi.fn(async () => {}),
      resolveApproval: vi.fn(),
      onEvent: vi.fn((next: (event: AgentEvent) => void) => {
        listeners.add(next)
  ...
  function transferSnapshot() {
    ...
    if (snapshot.project) snapshot.project.machineId = snapshot.machine.id
    const streaming = snapshot.sessions[0]!
    const moved = snapshot.sessions[1]!
    ...
    streaming.providerThreadId = "thread-streaming"
    moved.providerThreadId = "thread-moved"
    return { snapshot, streaming, moved }
  }

  function observingClient(socket: WebSocket) {
    const responses = new Map<number, (message: Record<string, unknown>) => void>()
    ...
    socket.on("message", (data) => {
      ...
      if (message.id !== undefined) responses.get(message.id)?.(message)
      else if (message.method === "workspace.delta") deltas.push(message.params ?? {})
    })
    let nextId = 0
    const rpc = (method: string, params: Record<string, unknown>) =>
      new Promise<Record<string, unknown>>((resolve) => {
  ...
  function settles<T>(promise: Promise<T>, withinMs: number): Promise<boolean> {
    return Promise.race([
  ...
  it("keeps serving other sessions while a transfer dials and uploads", async () => {
    const { snapshot, streaming, moved } = transferSnapshot()
    const listeners = new Set<(event: AgentEvent) => void>()
    const targetMachineId = `machine-${"b".repeat(32)}`
    let dialStarted!: () => void
    let releaseDial!: () => void
    const dialing = new Promise<void>((resolve) => { dialStarted = resolve })
    const store = new SqliteWorkspaceStore(":memory:", snapshot)
    const transferScratch = await mkdtemp(join(tmpdir(), "domovoi-server-transfer-target-"))
    scratchDirectories.push(transferScratch)
    const targetTransactions = new FileTransferTransactions(transferScratch)
    ...
    const daemon = new DomovoiDaemon({
      ...
      workspaceService: {
        ...stubWorkspaceService(),
        pushSessionRef: async () => ({
          remote: "origin",
          ref: `refs/domovoi/sessions/${moved.id}`,
          commit: transferCheckpointCommit,
        }),
      },
      connectToMachine: async () => {
        ...
        if (connectionCount === 3) {
          dialStarted()
          await new Promise<void>((resolve) => { releaseDial = resolve })
        }
        return {
          call: async (method, params) => {
            ...
            if (method === "transfer.status") {
              return targetTransactions.status(
            ...
            if (method === "transfer.member") {
              return targetTransactions.acceptMember(params as never)
            }
            if (method === "transfer.commit") {
              ...
              return {
                ...
                workspacePath: `/worktrees/${manifest.sessionId}`,
    ...
    await identifyClient(socket)
    const { rpc, deltas } = observingClient(socket)
    await rpc("session.send", { sessionId: streaming.id, prompt: "go", client: "desktop" })
    ...
    await expect(Promise.race([
      dialing.then(() => "dialing" as const),
      transfer.then((answer) => ({ answeredBeforeDial: answer })),
    ])).resolves.toBe("dialing")
    for (const listener of listeners) {
      listener({
        type: "text-delta",
        threadId: "thread-streaming",
        turnId: "turn-streaming",
        delta: "streamed while another session moves",
      })
    }
    const deltaDelivered = await settles(
      vi.waitFor(() => expect(deltas).toHaveLength(1), { timeout: 300 }),
      400,
    )
    releaseDial()
    const answer = await transfer

    expect(deltaDelivered).toBe(true)
    expect(answer).toMatchObject({ result: { outcome: "succeeded" } })
    socket.close()
  })

  it("keeps serving other sessions while an arriving bundle restores", async () => {
    const { snapshot, streaming } = transferSnapshot()
    const listeners = new Set<(event: AgentEvent) => void>()
    const bytes = Buffer.from("PACK a session worktree")
    const peerMachineId = `machine-${"b".repeat(32)}`
    const incomingSnapshot = structuredClone(demoWorkspace)
    incomingSnapshot.machine.id = peerMachineId
    if (incomingSnapshot.project) incomingSnapshot.project.machineId = peerMachineId
    const incoming = incomingSnapshot.sessions.find((candidate) => candidate.state === "idle")!
    incoming.id = "session-arriving"
    incoming.state = "idle"
    incoming.workspacePath = "/source/session-arriving"
    incoming.baseCommit = transferBaseCommit
    incoming.ownershipGeneration = 0
    delete incoming.activeTurnId
    delete incoming.providerThreadId
    incomingSnapshot.sessions = [incoming]
    incomingSnapshot.activeSessionId = incoming.id
    incomingSnapshot.approvals = []
    incomingSnapshot.thread = []
    incomingSnapshot.artifacts = []
    incomingSnapshot.workingPlans = []
    incomingSnapshot.annotations = []
    const intent = await prepareSessionTransferIntent({
      snapshot: incomingSnapshot,
      sessionId: incoming.id,
      usage: [],
      sourceMachineId: peerMachineId,
      targetMachineId: snapshot.machine.id,
      sourceProjectId: incomingSnapshot.project!.id,
      targetProjectId: snapshot.project!.id,
      lineageCommit: transferBaseCommit,
      sourceHeadCommit: transferBaseCommit,
      worktreeDigest: `sha256:${"e".repeat(64)}`,
      method: "git-bundle",
      readIgnoredArtifactSource: async () => undefined,
      readAnnotationCrop: async () => { throw new Error("No annotation crops") },
    })
    const packaged = createSessionTransferPackage(intent, {
      transferId: `transfer-${"c".repeat(32)}`,
      checkpointCommit: transferCheckpointCommit,
    ...
    const store = new SqliteWorkspaceStore(":memory:", snapshot)
    const peerCredential = store.devices.pair({
      label: "source daemon",
      binding: { kind: "machine", machineId: peerMachineId },
    }).token
    const daemon = new DomovoiDaemon({
      port: 0,
      statePath: ":memory:",
    ...
    const peer = new WebSocket(`ws://${address.host}:${address.port}/rpc`, {
    ...
    const peerRpc = rpcCaller(peer)
    await expect(peerRpc("system.hello", {
      client: "machine",
      clientVersion: "0.0.1",
      protocolVersion,
    })).resolves.toHaveProperty("result")
    await expect(peerRpc("transfer.preflight", {
      ...
      lineageCommit: packaged.manifest.project.lineageCommit,
    ...
    for (const listener of listeners) {
      listener({
        ...
        threadId: "thread-streaming",
        turnId: "turn-streaming",
        delta: "streamed while a session arrives",
      })
    }
    const deltaDelivered = await settles(
      vi.waitFor(() => expect(deltas).toHaveLength(1), { timeout: 300 }),
    ...
    expect(deltaDelivered).toBe(true)
    expect(answer).toMatchObject({ result: { state: "committed" } })
    peer.close()
    socket.close()
  })
  ...
  it("refuses a transfer from a client holding only a device credential", async () => {
    ...
    const call = rpcCaller(socket)
    await expect(call("system.hello", {
      client: "desktop",
      clientVersion: "0.0.1",
      protocolVersion,
    })).resolves.toHaveProperty("result")
