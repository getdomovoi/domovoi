...
import { fleetSnapshotSchema, workspaceSnapshotSchema } from "@getdomovoi/protocol"
...
it("advertises this build in production workspace and fleet facts", async () => {
  ...
  const workspace = workspaceSnapshotSchema.parse(await daemon.root.ok("workspace.get", {}))
  expect(workspace.machine.version).toBe("9.8.7-test")
  expect(remote(fleetSnapshotSchema.parse(await daemon.root.ok("fleet.list", {})), daemon.id).version).toBe("9.8.7-test")
})
...
it("refreshes persisted local build identity on restart without changing machine identity", async () => {
  ...
  const current = workspaceSnapshotSchema.parse(await restarted.root.ok("workspace.get", {}))
  expect(current.machine).toMatchObject({ id: daemon.id, version: "9.8.7-test" })
  expect(remote(fleetSnapshotSchema.parse(await restarted.root.ok("fleet.list", {})), daemon.id).version).toBe("9.8.7-test")
