...
describe("relay dispatcher inheritance", () => {
  ...
  it("keeps terminal writes owned by the admitted socket and reaps after channel close", async () => {
    ...
    const paired = store.devices.pair({ label: "operator desktop", binding: { kind: "client", client: "desktop" }, channelPublicKey: relayPublicKeyFromPrivateKey(key) })
    const owner = connect(daemon, paired.token), other = connect(daemon, paired.token)
    await owner.hello({ client: "desktop" }); await other.hello({ client: "desktop" })
    const terminalId = "terminal-relay-proof"
    expect(await owner.rpc("terminal.create", { terminalId, sessionId: session.id, cols: 80, rows: 24, client: "desktop", clientId: paired.device.id })).toHaveProperty("result")
    expect(await owner.rpc("terminal.input", { terminalId, data: "owned", client: "desktop", clientId: paired.device.id })).toHaveProperty("result")
    expect(terminal.write).toHaveBeenCalledWith("owned")
    expect(await other.rpc("terminal.input", { terminalId, data: "not owned", client: "desktop", clientId: paired.device.id })).toHaveProperty("error")
