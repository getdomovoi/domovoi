...
import {
  localOwnerIdentitySchema, localOwnerSecretSchema, localOwnerProof, verifyLocalOwnerProof,
...
describe("local owner challenge", () => {
  ...
  it.each(["01.2.3", "1.2.3\n", "1".repeat(61) + ".0.0"])("refuses malformed identity version %j before signing", (protocolVersion) => {
    const malformed = { ...identity, protocolVersion }
    expect(localOwnerIdentitySchema.safeParse(malformed).success).toBe(false)
    expect(() => localOwnerProof(key, malformed, nonce)).toThrow()
  })
