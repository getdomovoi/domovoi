...
import { serviceConfigurationPath, serviceRegistrationBlocksProfile } from "./service/configuration.js"
...
export function runProfileCommand(args: readonly string[], input: {
  ...
  try {
    ...
    const profile = profileLocation(input.homeDirectory, input.profileDirectory)
    ...
    if (serviceRegistrationBlocksProfile(input.homeDirectory, profile)) throw new Error(`Saved service configuration still exists at ${configuration}. Run domovoid service remove before confirming that no supervisor will restart the daemon.`)
    ...
    deadline.throwIfExpired()
    writeLocalOwnerRemovalReceipt(profile, lease, receipt, deadline)
    input.stdout(`Recorded your no-supervisor confirmation for owner instance ${record.instanceId} at ${localOwnerRemovalReceiptPath(profile)}. No daemon was started. Reopen Desktop to recover this profile.\n`)
    return 0
  } catch (error) {
