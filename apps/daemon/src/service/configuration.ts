...
export function parseServiceConfiguration(text: string): ServiceConfiguration {
  try {
    ...
    const { tls, advertiseHost, tailnetHost, sshTunnels, allowedOrigins, registrationId, relayIdentityPublicKey, relayCredentialFile, profileDirectory, wsl, serviceRuntime, ...required } = configurationSchema.parse(JSON.parse(text))
