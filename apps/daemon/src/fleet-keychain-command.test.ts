...
describe("local fleet keychain recovery", () => {
  it("lists the complete native index beyond the wire cap without reading credential bytes", async () => {
    const f = fixture()
    const readSecret = vi.spyOn(f.credentials, "forMachine")
    expect(await runFleetKeychainCommand(["fleet-keychain", "list"], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(0)
  ...
  it("requires a stopped-daemon confirmation before removing only the named local key and index entry", async () => {
    const f = fixture(2)
    expect(await runFleetKeychainCommand(["fleet-keychain", "forget", f.ids[0]!], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(1)
    expect(f.credentials.forMachine(f.ids[0]!)).toBe("n".repeat(43))
    expect(await runFleetKeychainCommand(["fleet-keychain", "forget", f.ids[0]!, "--confirm-daemon-stopped"], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(0)
  ...
  it("refuses invalid identities and never echoes argv or keychain error secrets", async () => {
    const f = fixture(1)
    const remove = vi.spyOn(f.credentials, "forget")
    expect(await runFleetKeychainCommand(["fleet-keychain", "forget", "token=do-not-print", "--confirm-daemon-stopped"], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(1)
    expect(remove).not.toHaveBeenCalled()
    vi.spyOn(f.credentials, "machines").mockImplementation(() => { throw new Error("token=do-not-print") })
    expect(await runFleetKeychainCommand(["fleet-keychain", "list"], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(1)
    expect(JSON.stringify([...f.stdout.mock.calls, ...f.stderr.mock.calls])).not.toContain("do-not-print")
  })

  it("does not claim removal when keychain readback still finds the credential", async () => {
    const f = fixture(1)
    vi.spyOn(f.credentials, "forget").mockImplementation(() => {})
    expect(await runFleetKeychainCommand(["fleet-keychain", "forget", f.ids[0]!, "--confirm-daemon-stopped"], { ...f, credentials: asyncTestCredentials(f.credentials) })).toBe(1)
