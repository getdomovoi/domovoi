...
describe("turn-local image validation", () => {
  it("passes bounded JPEG bytes without inventing an annotation", () => {
    expect(prepareSessionAttachments([upload(jpeg)], { vision: true })).toEqual([
      { attachmentIndex: 0, mimeType: "image/jpeg", bytes: jpeg },
    ])
    expect(prepareSessionAttachments([], undefined)).toEqual([])
    expect(prepareSessionAttachments(undefined, undefined)).toEqual([])
  })
  ...
  it.each([
    ...
    expect(() => prepareSessionAttachments([upload(Buffer.from(bytes))], { vision: true })).toThrow(/bounded PNG or JPEG/)
  })
  ...
  it("enforces bounds again before decoding at the daemon boundary", () => {
    for (const entry of [{ ...upload(jpeg), data: "AB==" },
      ...
      expect(() => prepareSessionAttachments([entry], { vision: true })).toThrow(/bounded PNG or JPEG/)
    }
    expect(() => prepareSessionAttachments([upload(jpeg), upload(jpeg), upload(jpeg)], { vision: true })).toThrow(/bounded PNG or JPEG/)
    const tooWide = Buffer.from(jpeg)
    tooWide.writeUInt16BE(2049, 14)
    expect(() => prepareSessionAttachments([{ ...upload(tooWide), width: 2049 }], { vision: true })).toThrow(/bounded PNG or JPEG/)
  })
})
