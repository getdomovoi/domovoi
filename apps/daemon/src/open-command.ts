...
import { repeatableOpenMessages } from "./rpc-errors.js"
...
// read out of a distribution, and so are the daemon's fixed answers for a
// folder it cannot open as a repository; anything else from the daemon can
// quote a path someone is watching the screen for, so only the fact of the
// failure is reported.
function isNamedRefusal(error: unknown): error is Error {
  return error instanceof WslError
    || (error instanceof Error && (/domovoid|wsl\.exe/.test(error.message) || repeatableOpenMessages.has(error.message)))
}
