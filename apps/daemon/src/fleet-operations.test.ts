...
describe("durable fleet operations", () => {
  ...
  it("makes forget visible while its keychain deletion is unfinished and resumes after restart", () => {
    ...
    expect(fleet.pendingForgetEnrollment(forgetting.id)).toEqual({ facts, credentialDigest: digest })
    expect(fleet.pendingForgetEnrollment(enrollment.id)).toBeUndefined()
    const restarted = new SqliteFleetRegistry(database)
    ...
    expect(restarted.pendingForgetEnrollment(forgetting.id)).toBeUndefined()
