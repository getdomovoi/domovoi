...
export async function runFleetKeychainCommand(
  args: string[],
  input: { credentials: AsyncMachineCredentials; stdout(text: string): void; stderr(text: string): void },
): Promise<number> {
  ...
  try {
    if (list) {
      ...
      const ids = machineIdSchema.array().parse(await input.credentials.machines(deadline))
      ...
      await input.credentials.forget(id, deadline)
      if (await input.credentials.forMachine(id, deadline) !== undefined || (await input.credentials.machines(deadline)).includes(id)) {
