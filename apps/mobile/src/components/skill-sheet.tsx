      ...
      <View className="flex-1 justify-end bg-desk/80">
        <View className="max-h-[80%] gap-3 rounded-t-2xl border-t border-border bg-card p-5 pb-8">
          <View className="flex-row items-center gap-2">
            <Text variant="nav" className="flex-1">Skills for this turn</Text>
            <Button title="Done" variant="ghost" onPress={onClose} className="px-2" />
          </View>

          {problem ? <Text variant="note" className="text-destructive">{problem}</Text> : null}
          ...
          <ScrollView contentContainerClassName="gap-1.5">
            {rows.map((row) => (
              <Pressable
                ...
                className="min-h-tap flex-row items-center gap-3 rounded-xl border border-border bg-background px-3 py-2.5 active:opacity-70"
              >
                <View className={cn(
                  "h-[18px] w-[18px] items-center justify-center rounded-md border",
                  row.selected ? "border-primary bg-primary" : "border-border",
                )}>
                  {/* Neither loaded face has a glyph for a tick, so the mark is
                      drawn rather than typed. */}
                  {row.selected ? <Icon name="check" tone="primary-foreground" size={12} /> : null}
                </View>
                <View className="flex-1 gap-0.5">
                  <Text variant="machine" className="text-[11px] text-foreground">{row.name}</Text>
                  {row.description
                    ? <Text variant="note" numberOfLines={2}>{row.description}</Text>
                    : null}
                </View>
              </Pressable>
            ))}
