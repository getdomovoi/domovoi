...
// Reasoning effort lives in the model menu as its own group: the selected
// model's efforts as it reported them, its default marked, the session's
// current effort ticked. A pick goes out through the same onChange the model
// switch uses, which the composer sends as session.setRuntime.
function withEfforts(supportedReasoningEfforts: string[], defaultReasoningEffort: string): Record<string, ProviderModel[]> {
  return {
    ...catalogs,
    "claude-code": [
      { ...catalogs["claude-code"]![0]!, supportedReasoningEfforts, defaultReasoningEffort },
      catalogs["claude-code"]![1]!,
    ],
  }
}

it("draws a Reasoning group for the selected model's efforts, with its default marked", async () => {
  const user = userEvent.setup()
  const reported = withEfforts(["low", "medium", "high", "max"], "high")
  render(popover({ onListModels: vi.fn(async (provider: string) => reported[provider] ?? []) }))
  await user.click(screen.getByRole("button", { name: /claude-code · sonnet 4\.6/ }))
  await settle()
  const group = screen.getByRole("radiogroup", { name: "Reasoning effort" })
  const rows = within(group).getAllByRole("radio")
  expect(rows.map((row) => row.getAttribute("aria-label"))).toEqual(["low", "medium", "high, the model's default", "max"])
  expect(rows.map((row) => row.getAttribute("aria-checked"))).toEqual(["false", "true", "false", "false"])
  expect(within(rows[2]!).getByText("default")).toBeTruthy()
  expect(screen.getByText("Applies from the next turn. A turn already in flight keeps the effort it started with.")).toBeTruthy()
})

it("sends a picked effort through the same change the model switch uses", async () => {
  const user = userEvent.setup()
  const onChange = vi.fn()
  const reported = withEfforts(["low", "medium", "high"], "medium")
  render(popover({ onChange, onListModels: vi.fn(async (provider: string) => reported[provider] ?? []) }))
  await user.click(screen.getByRole("button", { name: /claude-code · sonnet 4\.6/ }))
  await settle()
  await user.click(within(screen.getByRole("radiogroup", { name: "Reasoning effort" })).getByRole("radio", { name: "high" }))
  expect(onChange).toHaveBeenCalledTimes(1)
  expect(onChange).toHaveBeenCalledWith({ ...runtime, reasoning: "high" })
  expect(screen.queryByRole("radiogroup", { name: "Reasoning effort" })).toBeNull()
})

it("draws no Reasoning group for a model that reports no efforts", async () => {
  const user = userEvent.setup()
  const reported = withEfforts([], "medium")
  render(popover({ onListModels: vi.fn(async (provider: string) => reported[provider] ?? []) }))
  await user.click(screen.getByRole("button", { name: /claude-code · sonnet 4\.6/ }))
  await settle()
  expect(screen.getAllByRole("option").length).toBeGreaterThan(0)
  expect(screen.queryByRole("radiogroup", { name: "Reasoning effort" })).toBeNull()
  expect(screen.queryByText(/keeps the effort it started with/)).toBeNull()
})

it("holds the effort rows while a runtime update is in flight", async () => {
  const user = userEvent.setup()
  const onChange = vi.fn()
  const reported = withEfforts(["low", "medium", "high"], "medium")
  const onListModels = vi.fn(async (provider: string) => reported[provider] ?? [])
  const view = render(popover({ onChange, onListModels }))
  await user.click(screen.getByRole("button", { name: /claude-code · sonnet 4\.6/ }))
  await settle()
  view.rerender(popover({ onChange, onListModels, pending: true }))
  const high = within(screen.getByRole("radiogroup", { name: "Reasoning effort" })).getByRole("radio", { name: "high" })
