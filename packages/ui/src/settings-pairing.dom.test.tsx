...
it("hosts the pairing card under Phone and tablet and sends device management to Machines", async () => {
  ...
  render(<SettingsShell
    ...
    pairing={{ onIssueCode: vi.fn(async () => ({ code: "hearth-quiet-ember-42", expiresAt: new Date().toISOString() })), onCopy: vi.fn(async () => {}), onListDevices: vi.fn(async () => ({ devices: [{ id: `device-${"d".repeat(32)}`, label: "iPhone", pairedAt: "2026-09-01T00:00:00.000Z", binding: { kind: "client" as const, client: "phone" as const, clientAccess: "full" as const } }] })) }}
  />)
