...
it("hosts the pairing card under Phone and tablet and sends device management to Machines", async () => {
  ...
  render(<SettingsShell
    ...
    pairing={{ connected: true, onIssueCode: vi.fn(async () => ({ code: "hearth-quiet-ember-42", expiresAt: new Date().toISOString(), pairingAddress: { url: "wss://mac-mini-m4.tail4c2e.ts.net:47831/rpc", loopback: false } })), onCopy: vi.fn(async () => {}), onListDevices: vi.fn(async () => ({ devices: [{ id: `device-${"d".repeat(32)}`, label: "iPhone", pairedAt: "2026-09-01T00:00:00.000Z", binding: { kind: "client" as const, client: "phone" as const, clientAccess: "full" as const } }] })) }}
  />)
...
it("offers no pairing code while the daemon connection is closed", async () => {
  render(<SettingsShell
    providers={[]} secrets={[]} approvalRules={[]}
    notifications={{ completion: true, failure: true, approvalNeeded: true }} onNotificationsChange={vi.fn()}
    onOpenFleet={vi.fn()} onOpenSkills={vi.fn()} onOpenAudit={vi.fn()} theme="system" onThemeChange={vi.fn()}
    pairing={{ connected: false, onIssueCode: vi.fn(), onCopy: vi.fn(async () => {}), onListDevices: vi.fn(async () => ({ devices: [] })) }}
  />)
