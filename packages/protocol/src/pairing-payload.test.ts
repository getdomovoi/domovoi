...
const payload = { v: 1 as const, url: "wss://djs-macbook-pro-1.raptor-pompano.ts.net:47831/rpc", code, label: "djs-macbook-pro-1" }

// The QR a phone scans carries the daemon's address and a single-use pairing
// code, nothing else: no credential is ever drawn on a screen. The text is
// self-describing so a scanner that reads some other QR can say so, and the
// code is validated as one before the phone dials anything.
describe("pairing payload", () => {
  it("round-trips through the text a QR carries", () => {
    ...
    // The code is in the encoded text, not readable beside it.
  ...
  it("names what is wrong with text that is not a pairing code", () => {
    ...
    const forged = `${pairingPayloadPrefix}${Buffer.from(JSON.stringify({ ...payload, code: "not a code" })).toString("base64url")}`
    expect(() => decodePairingPayload(forged)).toThrow(/no code the machine would take/)
    // A payload carrying a credential is the shape this replaced, and is not
    // read as one that happens to be missing a code.
    const credentialBearing = `${pairingPayloadPrefix}${Buffer.from(JSON.stringify({ v: 1, url: payload.url, token: "t".repeat(43) })).toString("base64url")}`
    expect(() => decodePairingPayload(credentialBearing)).toThrow()
  ...
  it("refuses a plaintext address off loopback and any extra field", () => {
    ...
    expect(pairingPayloadSchema.safeParse({ ...payload, token: "t".repeat(43) }).success).toBe(false)
