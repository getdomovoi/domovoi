...
test("names the numbers that were missing", () => {
  assert.equal(
    driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: 3, behind: 0, mainAgeDays: 1, staleLocal: 0 }),
...
test("calls out the crossing rather than every number", () => {
  const quiet = driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: stackThreshold, behind: 0, mainAgeDays: 1, staleLocal: 0 })
  assert.doesNotMatch(quiet, /stack of smaller pull requests/)

  const loud = driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: 120, behind: 0, mainAgeDays: 3, staleLocal: 0 })
...
test("reports how far behind main it has fallen, and stays silent when it has not", () => {
  assert.match(
    driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: 2, behind: 9, mainAgeDays: 0, staleLocal: 0 }),
    /2 commits ahead of origin\/main, 9 behind/,
  )
  assert.doesNotMatch(
    driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: 2, behind: 0, mainAgeDays: 0, staleLocal: 0 }),
    /behind/,
  )
})
...
test("says when local main is too stale to have been the baseline", () => {
  assert.match(
    driftReport({ detached: false, branch: "feat/one", baseline: "origin/main", ahead: 62, behind: 0, mainAgeDays: 1, staleLocal: 60 }),
