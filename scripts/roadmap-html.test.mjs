...
test("renders a full page with the title from the first heading and no timestamp", () => {
  ...
  assert.ok(html.includes('<div class="meta">SHIP-PLAN.md  ·  generated by scripts/roadmap-html.mjs</div>'))
  assert.ok(html.includes('<main class="doc">\n<h1 id="domovoi-roadmap">Domovoi roadmap</h1>\n<p>Intro &amp; more.</p>\n'))
  assert.ok(html.endsWith("</main>\n</div>\n</body>\n</html>\n"))
  assert.doesNotMatch(html, /\d{4}-\d{2}-\d{2} \d{2}:\d{2}/)
})

test("reports a stale SHIP-PLAN.html and accepts a regenerated one", async (t) => {
  const root = await fixture({ "SHIP-PLAN.md": "# Domovoi roadmap\n\n- [x] done\n" })
  t.after(() => rm(root, { recursive: true, force: true }))

  assert.deepEqual(await checkRoadmapHtml(root), ["SHIP-PLAN.html: missing; run pnpm roadmap:html"])

  await writeRoadmapHtml(root)
  assert.deepEqual(await checkRoadmapHtml(root), [])
  assert.equal(await readFile(join(root, "SHIP-PLAN.html"), "utf8"), renderRoadmapHtml("# Domovoi roadmap\n\n- [x] done\n"))

  await writeFile(join(root, "SHIP-PLAN.md"), "# Domovoi roadmap\n\n- [ ] done\n")
  assert.deepEqual(await checkRoadmapHtml(root), ["SHIP-PLAN.html: stale relative to SHIP-PLAN.md; run pnpm roadmap:html"])
})
