...
async function inspectCanonicalRelease(release, { request, deadline }) {
  ...
  if (record) for (const file of release.files) {
    const asset = record.assets?.find((item) => item.name === file.name)
    if (asset && asset.digest !== `sha256:${file.sha256}`) throw new Error(`Release asset ${file.name} has a different or missing checksum; it will not be overwritten`)
    if (!asset && !record.draft) throw new Error(`Published release is missing ${file.name}; inspect it before repairing a public release`)
  }
  return { reference, record }
}

// Refuse known conflicts before npm becomes immutable. These observations
// are not a cross-service lock: publication checks again after npm succeeds.
export async function preflightCanonicalRelease(release, { request, readVersion, deadline }) {
  await inspectCanonicalRelease(release, { request, deadline })
  for (const pkg of release.packages) {
    const published = await deadline.run(() => readVersion(pkg))
    if (published !== undefined) requirePublishedArtifact(pkg, published)
  }
  return { state: "checked", tag: release.gitTag, commit: release.commit }
}
...
export async function publishCanonicalRelease(release, { request, readVersion, upload, notes, deadline }) {
  const call = (...args) => deadline.run(() => request(...args))
  for (const pkg of release.packages) requirePublishedArtifact(pkg, await deadline.run(() => readVersion(pkg)))
  let { reference, record } = await inspectCanonicalRelease(release, { request, deadline })
  ...
  if (!record) {
    record = await call("/releases", "POST", { tag_name: release.gitTag, target_commitish: release.commit,
      name: `Domovoi ${release.version}`, body: `Source commit: ${release.commit}\n\n${notes}`, draft: true,
      prerelease: release.prerelease, make_latest: "false" })
  }
  for (const file of release.files) {
    const existing = record.assets?.find((asset) => asset.name === file.name)
    if (!existing) await deadline.run(() => upload(record.id, file))
  }
...
export async function releaseGithub({ root = process.cwd(), env = process.env, operation = "publish" } = {}) {
  ...
  if (!["check", "publish"].includes(operation)) throw new Error("Expected a release check or publish operation")
  const deadline = operation === "check"
    ? bootstrapDeadline(60_000, "Release preflight exceeded one minute; no publication was admitted")
    : bootstrapDeadline(600_000, "GitHub release exceeded ten minutes; inspect npm, the tag and any draft before retrying")
  try {
    ...
    if (operation === "check") return await preflightCanonicalRelease(release, { request, readVersion, deadline })
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    if (!["check", "publish"].includes(operation)) throw new Error("usage: node scripts/release-github.mjs <check|publish>")
    console.log(JSON.stringify(await releaseGithub({ operation }), null, 2))
  }
