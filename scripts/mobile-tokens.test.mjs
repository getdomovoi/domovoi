...
test("the check fails until the generated files match the stylesheet, then passes", async () => {
  ...
  assert.match(module, /module\.exports = \{ colors, radius, fontSize, fonts, fontFamily, shadows, outOfGamut, alphaSteps, withAlpha \}\n$/)
...
// The phone's ramp is its own, not the desktop's scaled down, so the generator
// reads --text-phone-* rather than deriving anything from --text-micro.
test("pairs a phone type role with its line height", () => {
  const scale = phoneTypeScale({
    "text-phone-body": "12.5px",
    "text-phone-body-lh": "19px",
    "text-phone-machine": "10px",
    "radius": "0.65rem",
  })
  assert.deepEqual(scale, { body: ["12.5px", "19px"], machine: "10px" })
})

// A line height whose size was renamed or removed would silently produce a role
// Tailwind cannot size, so it is an error rather than a dropped pair.
test("refuses a line height with no size beside it", () => {
  assert.throws(
    () => phoneTypeScale({ "text-phone-body-lh": "19px", "text-phone-machine": "10px" }),
    /--text-phone-body-lh has no --text-phone-body/,
  )
})

test("refuses a stylesheet that declares no phone roles", () => {
  assert.throws(() => phoneTypeScale({ "text-micro": "10.5px" }), /declares no --text-phone-\* roles/)
})
