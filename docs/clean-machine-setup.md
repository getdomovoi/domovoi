...
- The [dedicated WSL job](wsl-ci.md) provisions one WSL 2 guest and requires real discovery
  and path-boundary proofs, including a non-default Windows-drive mount root. A successful
  native run is required evidence, not the workflow's existence. Authenticated distro daemon
  access, repository work and daemon restart are not yet proven by that job.
