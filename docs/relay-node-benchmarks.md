...
| Extra native crypto packages/addons in this experiment | None | None |
...
`node_shared_openssl: false`. No additional native addon is loaded for either
suite. Replacing the experimental state machine with a Rust/Noise binding would
...
1.24 ms and 2.01 ms in the final-source run tabulated above. The crypto and harness
...
are generated by [node-benchmark.ts](../packages/protocol/experimental/relay/node-benchmark.ts).
